...
class constructFile:
	...
	def write_incoming(self):
		fp = open(self.module_name, 'a+')
		fp.write('\tdef incoming(self, message, bytes, ip, vuLogger, random_reply):\n')
		fp.write('\t\ttry:\n')
		...
		if self.vdict['default_reply']=="random" or self.vdict['default_reply']==None:
			...
			#fp.write('\t\t\t\t\tself.reply.append( struct.pack("B", random.randint(0,255)) )\n')
		else:
			fp.write('\t\t\tself.reply = []\n')
			fp.write('\t\t\tfor i in range(0,510):\n')
			fp.write('\t\t\t\ttry:\n')
	...
	def write_stages(self):
		...
		for i in range(1, number_of_stages+1):
			try:
				if i == 1:
					if list_of_stages[i]['bytes_to_read'] != "":
						fp.write('\t\t\tif self.stage == "%s_STAGE%i" and bytes == %s:\n' % (self.v_name, i, list_of_stages[i]['bytes_to_read']))
					else:
						fp.write('\t\t\tif self.stage == "%s_STAGE%i":\n' % (self.v_name, i))
				else:
					if list_of_stages[i]['bytes_to_read'] != "":
						fp.write('\t\t\telif self.stage == "%s_STAGE%i" and bytes == %s:\n' % (self.v_name, i, list_of_stages[i]['bytes_to_read']))
					else:
