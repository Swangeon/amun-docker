...
class vuln:
	...
	def incoming(self, message, bytes, ip, vuLogger, random_reply, ownIP):
		try:
			...
			if self.stage=="NETDDE_STAGE1" and (bytes==72 or bytes==82 or bytes==84 or bytes==88 or bytes==80 or bytes==4 or bytes==90 or bytes==86 or bytes==69):
				...
				return resultSet
			elif self.stage=="NETDDE_STAGE2" and (bytes==174 or bytes==137 or bytes==52 or bytes==51 or bytes==141 or bytes==148 or bytes==4 or bytes==133 or bytes==53):
			...
			elif self.stage=="NETDDE_STAGE3" and (bytes==79 or bytes==4 or bytes==133 or bytes==137 or bytes==76):
				...
				return resultSet
			elif self.stage=="NETDDE_STAGE3" and bytes==1024:
				resultSet['result'] = True
				resultSet['accept'] = True
				reply = self.smbHandler.consume(message, ownIP)
				if reply!=None:
				...
				return resultSet
			elif self.stage=="NETDDE_STAGE4" and bytes==1024:
				resultSet['result'] = True
				resultSet['accept'] = True
				reply = self.smbHandler.consume(message, ownIP)
				if reply!=None:
