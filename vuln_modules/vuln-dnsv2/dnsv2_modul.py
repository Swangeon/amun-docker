...
import struct
import random
import dnsv2_shellcodes

class vuln:
	def __init__(self):
		try:
			self.vuln_name = "DNSV2 Vulnerability"
			self.stage = "DNSV2_STAGE1"
	...
	def incoming(self, message, bytes, ip, vuLogger, random_reply, ownIP):
		try:
			...
			if self.stage=="DNSV2_STAGE1" and bytes==137:
				if dnsv2_shellcodes.dnsv2_request_stage1==message:
					resultSet['result'] = True
					resultSet['accept'] = True
					self.reply[9] = "\x00"
					resultSet['reply'] = "".join(self.reply)
					self.stage = "DNSV2_STAGE2"
					return resultSet
			elif self.stage=="DNSV2_STAGE2" and bytes==168:
				if dnsv2_shellcodes.dnsv2_request_stage2==message:
					resultSet['result'] = True
					resultSet['accept'] = True
					self.reply[9] = "\x00"
					self.reply[95] = "\x05"
				...
				if bytes>0:
					#print ".::[Amun - DNSV2] collecting shellcode: %s ::." % (bytes)
					resultSet['result'] = True
					resultSet['accept'] = True
					resultSet['reply'] = "".join(self.reply)
					...
					#print ".::[Amun - DNSV2] finish collecting shellcode (bytes %s) ::." % (bytes)
					resultSet['result'] = False
					resultSet['accept'] = True
					resultSet['reply'] = "None"
					self.shellcode.append(message)
