...
import amun_logging
import random
import sub7_shellcodes

class vuln:
	def __init__(self):
		try:
			self.vuln_name = "SUB7 Vulnerability"
			self.stage = "SUB7_STAGE1"
			self.welcome_message = "PWD"
			self.shellcode = []
	...
	def incoming(self, message, bytes, ip, vuLogger, random_reply, ownIP):
		try:
			self.log_obj = amun_logging.amun_logging("vuln_sub7", vuLogger)
			...
			### PWD = Password
			if self.stage == "SUB7_STAGE1" and message[:3]=="PWD":
				resultSet["result"] = True
				resultSet["accept"] = True
				resultSet["reply"] = "You connected."
				self.stage = "SUB7_STAGE2"
				return resultSet
			elif self.stage == "SUB7_STAGE2" and message[:3]=="TID":
				resultSet["result"] = True
				resultSet["accept"] = True
				resultSet["reply"] = "UPS"
				self.stage = "SUB7_STAGE3"
				return resultSet
			elif self.stage == "SUB7_STAGE3" and message[:5]=="SFT05":
				resultSet["result"] = True
				resultSet["accept"] = True
