					...
					self.content_length = self.extract_length(v)
							...
							for item in self.header:
								if item.startswith('Location:'):
									...
									resultSet = self.shellcode_manager.start_matching( vulnResult, self.remote_ip, self.victimIP, 1, self.config_dict['replace_locals'], False)
...
class download_connectback(asyncore.dispatcher):
	...
	def check_shellcommands(self, commands):
		try:
			...
			result = self.shellcode_manager.start_shellcommand_matching(vulnResult, self._address[0], ownIP, ownPort, self.config_dict['replace_locals'], False)
			if result['result']:
